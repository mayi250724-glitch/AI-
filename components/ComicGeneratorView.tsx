...
import { ArrowLeft, BookOpen, Wand2, Download, Sparkles, AlertCircle, RefreshCw, Palette, Film } from 'lucide-react';
import * as GeminiService from '../services/geminiService';
import { ComicData, ComicPanel, StoryboardData } from '../types';
import FancyLoader from './FancyLoader';

interface ComicGeneratorViewProps {
  inputText: string;
  storyboardData?: StoryboardData | null; // NEW: Accept storyboard data
  onBack: () => void;
}

const ComicGeneratorView: React.FC<ComicGeneratorViewProps> = ({ inputText, storyboardData, onBack }) => {
  ...
  // Pre-fill prompt if using storyboard data (just for display mostly)
  useEffect(() => {
      if (storyboardData) {
          setPrompt(`[分镜转绘模式] 将当前 "${storyboardData.title}" 分镜脚本转换为漫画。\n包含 ${storyboardData.shots.length} 个镜头。`);
      } else {
          setPrompt(inputText);
      }
  }, [storyboardData, inputText]);
  ...
  const handleGenerate = async () => {
      ...
      try {
          ...
          if (storyboardData) {
              // Mode A: Adapt existing storyboard
              data = await GeminiService.adaptStoryboardToComic(storyboardData, style);
          } else {
              // Mode B: Generate from raw text
              if (!prompt.trim()) {
                  setIsAnalyzing(false);
                  return;
              }
              data = await GeminiService.generateComicScript(prompt, style);
          }
  ...
  const generatePanelImage = async (panel: ComicPanel, comicStyle: string) => {
      try {
          ...
          const imageUrl = await GeminiService.generateImage(fullPrompt, "3:4"); 
        ...
        <div className="h-16 border-b border-[#262626] bg-[#141414] px-6 flex items-center justify-between shrink-0 sticky top-0 z-50">
            <div className="flex items-center gap-4">
                ...
                <h2 className="text-lg font-bold flex items-center gap-2">
                    ...
                    {storyboardData ? "分镜转漫画 (Storyboard Adapter)" : "AI 漫画生成器"}
                </h2>
            </div>
        </div>
                 ...
                 <div className="flex flex-col md:flex-row gap-6">
                     <div className="flex-1 space-y-2">
                         <label className="text-sm font-bold text-gray-400">
                             {storyboardData ? "故事板来源 (只读)" : "漫画主题 / 故事梗概"}
                         </label>
                         {storyboardData ? (
                             <div className="w-full h-32 bg-[#0a0a0a] border border-[#333] rounded-lg p-4 flex flex-col justify-center gap-2">
                                 <div className="flex items-center gap-2 text-pink-400 font-bold">
                                     <Film size={18} />
                                     {storyboardData.title}
                                 </div>
                                 <p className="text-gray-400 text-sm line-clamp-2">{storyboardData.synopsis}</p>
                                 <div className="text-xs text-gray-600 font-mono mt-1">包含 {storyboardData.shots.length} 个分镜镜头</div>
                             </div>
                         ) : (
                             <textarea 
                                value={prompt}
                     ...
                     <div className="w-full md:w-72 space-y-4">
                         ...
                         <button 
                            ...
                            disabled={isAnalyzing || (!prompt.trim() && !storyboardData)}
                         ...
                         >
                            ...
                            {isAnalyzing ? "正在绘制..." : (storyboardData ? "开始转绘" : "生成漫画")}
                         </button>
                     </div>
                 </div>
            </div>
            ...
            {!comicData && !isAnalyzing && !errorMsg && (
                <div className="flex-1 flex flex-col items-center justify-center opacity-30">
                    ...
                    <p className="text-xl font-bold text-gray-500">
                        {storyboardData ? "准备将分镜转换为漫画..." : "等待灵感涌现..."}
                    </p>
                </div>
            )}
