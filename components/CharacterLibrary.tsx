...
const CharacterLibrary: React.FC<CharacterLibraryProps> = ({ characters, onUpdateCharacters, onSelect, onClose }) => {
  ...
  const handleGenerateImage = async () => {
    const basePrompt = newChar.visualPrompt || newChar.subjectDescription;
    if (!basePrompt) {
        alert("请先填写'画面提示词'或'主体描述'以生成图片。");
        return;
    }

    setIsProcessing(true);
    try {
      // Enhanced prompt for character sheet style (Chinese Three-View)
      const enhancedPrompt = `(人物三视图:1.5), (正面, 侧面, 背面), 全身展示, 角色设定图, ${newChar.name ? `名字: ${newChar.name},` : ''} ${basePrompt}, 高质量, 细节丰富, 白底`;
      
      // Explicitly call Nano Banana Pro model with 3:4 aspect ratio to match UI
      const imageUrl = await GeminiService.generateImage(enhancedPrompt, "3:4", "nano-banana-pro");
      
      setNewChar(prev => ({ ...prev, imageUrl: imageUrl }));
    } catch (e: any) {
      console.error(e);
      alert(`生成失败: ${e.message || "请检查网络或配置"}`);
    } finally {
