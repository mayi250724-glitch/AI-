...
import { ArrowLeft, Play, Pause, SkipForward, SkipBack, Film, Loader2, CheckCircle, AlertCircle, Download, RefreshCw, AlertTriangle, Scissors, Image as ImageIcon, Sparkles } from 'lucide-react';
import VideoGenerationModal from './VideoGenerationModal';
import VideoEditModal from './VideoEditModal'; // Imported
...
const FullMovieView: React.FC<FullMovieViewProps> = ({ shots, aspectRatio, onUpdateShot, onBack, onEdit, characters, onRegenerateImage }) => {
  ...
  const handleOpenVideoEdit = (e: React.MouseEvent) => {
      e.stopPropagation();
      const currentShot = shots[currentShotIndex];
      if (currentShot && currentShot.videoUrl) {
          setEditingVideoUrl(currentShot.videoUrl);
          setShowEditVideoModal(true);
          setIsPlaying(false); // Pause underlying player
      }
  };

  const handleVideoEditSuccess = (newUrl: string) => {
      const currentShot = shots[currentShotIndex];
      if (currentShot) {
          onUpdateShot(currentShot.id, { videoUrl: newUrl });
      }
  };
  ...
  const handleConfirmGenerateVideo = async (shotId: number, params: { prompt: string; firstFrameImage?: string; aspectRatio: string; model: string; lastFrameImage?: string; audioFile?: string; duration: number; subjectUrl?: string }) => {
      ...
      onUpdateShot(shotId, { isGeneratingVideo: true, generationProgress: 0, error: undefined, generationStatus: 'starting', audioFileUrl: params.audioFile });
      ...
      try {
        const videoUrl = await GeminiService.generateVideo(
            ...
            params.duration,
            undefined, // sourceVideoUrl
            params.subjectUrl, // subjectUrl (Face/Character Lock)
            params.audioFile // audioUrl
        );
  ...
  const handleRegenImage = (shot: Shot, e: React.MouseEvent) => {
      e.stopPropagation();
      onRegenerateImage(shot.id, shot.t2iPromptEn || shot.t2iPrompt);
  };

  const handleOpenVideoGen = (shot: Shot, e: React.MouseEvent) => {
      e.stopPropagation();
      openGenerationModal(shot);
  };
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white flex flex-col animate-fade-in">
      ...
      {/* Video Chat Edit Modal */}
      {showEditVideoModal && editingVideoUrl && (
          <VideoEditModal 
              isOpen={showEditVideoModal}
              onClose={() => setShowEditVideoModal(false)}
              sourceVideoUrl={editingVideoUrl}
      ...
      <div className="h-16 border-b border-[#262626] bg-[#141414] px-4 md:px-6 flex items-center justify-between shrink-0 sticky top-0 z-50">
        ...
        <div className="flex items-center gap-3 md:gap-6">
           ...
           >
             <Scissors size={16} /> <span className="hidden sm:inline">一键剪辑</span>
           </button>
      ...
      <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
        ...
        <div className="flex-1 bg-black relative flex flex-col order-1 lg:order-1 h-[50vh] lg:h-auto border-b lg:border-b-0 lg:border-r border-[#262626]">
            <div className="flex-1 flex items-center justify-center p-4 md:p-8 relative overflow-hidden group">
                {currentShot?.videoUrl ? (
                    <div className="relative max-w-full max-h-full shadow-2xl">
                        <video 
                            ...
                            onClick={togglePlay}
                        />
                        {/* Video Edit Overlay Button */}
                        <div className="absolute top-4 right-4 z-20 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                            <button 
                                onClick={handleOpenVideoEdit}
                                className="flex items-center gap-2 bg-purple-600/90 hover:bg-purple-500 text-white px-4 py-2 rounded-full shadow-lg backdrop-blur-md font-bold text-sm transform hover:scale-105 transition-all border border-purple-400/30"
                            >
                                <Sparkles size={16} /> AI 视频编辑
                            </button>
                        </div>
                    </div>
                ) : (
        ...
        <div className="w-full lg:w-80 h-full lg:h-auto border-t lg:border-t-0 bg-[#0f0f0f] flex flex-col order-2 lg:order-2">
            ...
            <div className="flex-1 overflow-y-auto">
                {shots.map((shot, idx) => (
                    <div 
                        key={shot.id}
                        className={`border-b border-[#222] transition-colors ${currentShotIndex === idx ? 'bg-[#222]' : 'hover:bg-[#1a1a1a]'}`}
                    >
                        ...
                        {/* Expanded Editor: Only for active shot */}
                        {currentShotIndex === idx && (
                            <div className="px-3 pb-3 space-y-3 animate-fade-in cursor-default" onClick={e => e.stopPropagation()}>
                                <div className="h-px bg-[#333] mb-2"></div>
                                
                                {/* T2I Prompt Edit */}
                                <div className="space-y-1">
                                    <div className="flex justify-between items-center">
                                        <label className="text-[10px] text-gray-500 font-bold flex items-center gap-1">
                                            <ImageIcon size={10} /> 文生图提示词 (T2I)
                                        </label>
                                        <button 
                                            onClick={(e) => handleRegenImage(shot, e)} 
                                            className="text-[10px] text-cinema-accent hover:text-white flex items-center gap-1 px-2 py-0.5 rounded hover:bg-[#333] transition-colors"
                                        >
                                            <RefreshCw size={10} /> 重新生成图片
                                        </button>
                                    </div>
                                    <textarea 
                                        className="w-full bg-[#0a0a0a] border border-[#333] rounded p-2 text-xs text-gray-300 focus:border-cinema-accent outline-none min-h-[60px] resize-none leading-relaxed"
                                        value={shot.t2iPromptEn || shot.t2iPrompt}
                                        onChange={(e) => onUpdateShot(shot.id, { t2iPromptEn: e.target.value })}
                                ...
                                <div className="space-y-1">
                                    <div className="flex justify-between items-center">
                                        <label className="text-[10px] text-gray-500 font-bold flex items-center gap-1">
                                            <Film size={10} /> 图生视频提示词 (I2V)
                                        </label>
                                        <button 
                                            onClick={(e) => handleOpenVideoGen(shot, e)} 
                                            className="text-[10px] text-purple-400 hover:text-white flex items-center gap-1 px-2 py-0.5 rounded hover:bg-[#333] transition-colors"
                                        >
                                            <Play size={10} /> 生成/重绘视频
                                        </button>
                                    </div>
                                    <textarea 
                                        className="w-full bg-[#0a0a0a] border border-[#333] rounded p-2 text-xs text-gray-300 focus:border-purple-500 outline-none min-h-[60px] resize-none leading-relaxed"
                                        value={shot.i2vPromptEn || shot.i2vPrompt}
                                        onChange={(e) => onUpdateShot(shot.id, { i2vPromptEn: e.target.value })}
