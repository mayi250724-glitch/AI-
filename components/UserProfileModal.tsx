...
import { X, LogOut, Phone, Mail, Award, Activity, Clock, FileText, ArrowRight, Trash2, Calendar, User, Loader2, Cloud } from 'lucide-react';
...
const UserProfileModal: React.FC<UserProfileModalProps> = ({ isOpen, onClose, user, onLogout, onLoadProject }) => {
  ...
  const isBackend = GeminiService.getApiConfig().enableBackend;
      ...
      <div className="relative bg-[#121212] w-full max-w-4xl h-[85vh] border border-[#262626] rounded-2xl shadow-2xl flex flex-col md:flex-row overflow-hidden">
        ...
        <div className="w-full md:w-64 bg-[#141414] border-r border-[#262626] flex flex-col">
            <div className="p-6 border-b border-[#262626] flex flex-col items-center">
                 ...
                 {isBackend && (
                     <div className="mt-2 text-[10px] bg-green-900/30 text-green-400 px-2 py-0.5 rounded border border-green-500/30 flex items-center gap-1">
                         <Cloud size={10} /> Enterprise
                     </div>
                 )}
            </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="bg-[#171717] border border-[#262626] rounded-xl p-6">
                                ...
                                <div className="flex justify-between items-center">
                                    <span className="text-lg font-mono text-gray-300">{user.phone}</span>
                                    {!isBackend && <button className="text-xs text-cinema-accent hover:underline">修改</button>}
                                </div>
                            </div>
                            <div className="bg-[#171717] border border-[#262626] rounded-xl p-6">
                                <h4 className="text-gray-500 text-xs uppercase font-bold tracking-wider mb-4 flex items-center gap-2">
                                ...
                                <div className="flex justify-between items-center">
                                    <span className="text-lg font-mono text-gray-300">{user.email}</span>
                                    {!isBackend && <button className="text-xs text-cinema-accent hover:underline">绑定</button>}
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="space-y-4">
                        {isLoading ? (
                            <div className="flex flex-col items-center justify-center py-20 gap-4">
                                ...
                                <span className="text-gray-500 text-xs">Syncing with database...</span>
                            </div>
                        ) : projects.length === 0 ? (
